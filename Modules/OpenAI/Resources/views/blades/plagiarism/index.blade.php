...
@php
    $pageLeft = 0;
    if ($userSubscription && in_array($userSubscription->status, ['Active', 'Cancel'])  && array_key_exists('page', $featureLimit)) {
        $pageLeft = $featureLimit['page']['remain'] ?? 0;  // Default to 0 if 'remain' is null
        $pageLimit = $featureLimit['page']['limit'] ?? 0;
    }
@endphp
