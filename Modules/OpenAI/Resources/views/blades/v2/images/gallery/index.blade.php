...
@endsection
@php
    $imageToggleFavoriteUrl = route('user.image.favourite');
...
<div class="dark:bg-[#292929] bg-[#F6F3F2] sidebar-scrollbar overflow-auto flex flex-col flex-1 font-Figtree border-l dark:border-[#474746] border-color-DF h-screen">
    <div class="9xl:px-[185px] 7xl:px-[140px] px-5 pt-[74px] 9xl:pb-[22px] pb-28">
        <div class="flex justify-between items-center gap-3 flex-wrap mt-[9px]">
            ...
            <a class="magic-bg rounded-xl text-base text-white justify-center items-center font-semibold py-3 flex text-center mt-4 cursor-pointer font-Figtree w-max px-[21px] whitespace-nowrap magic-shadow" href="{{ route('user.image.create') }}">
                                ...
                                @forelse ($images as $image)
                                    <tr class="gallery-item border-b dark:border-[#474746]" id="image_{{ $image->id }}">
                                        ...
                                        <td class="text-14 font-Figtree py-[18px] text-color-14 dark:text-white font-medium md:pl-[34px] md:pr-6 px-3 docs-table-row-one">
                                            ...
                                            <p class="xl:hidden mt-2 break-words">{{ data_get($image->generation_options,'size', 'None') }}</p>
                                            <p class="xl:hidden mt-2 break-words">{{ timeZoneFormatDate($image->created_at) }}</p>
                                        </td>
                                        <td class="text-13 font-Figtree py-[22px] text-color-89 font-medium px-6 w-64 hidden xl:table-cell">
                                            {{ data_get($image->generation_options,'size', 'None') }}
                                        </td>
                                        <td class="text-13 font-Figtree py-[22px] text-color-89 font-medium px-6 w-64 hidden xl:table-cell whitespace-nowrap">
                                            {{ timeZoneFormatDate($image->created_at) }}
                                        </td>
                                        <td
                                            ...
                                            <div class="gap-4 justify-end items-center hidden xl:flex">
                                                ...
                                                <div class="relative">
                                                    <a class="file-need-download image-tooltip-download flex items-center border border-color-89 dark:border-color-47 text-color-14 dark:text-white bg-white dark:bg-color-47 p-2 rounded-lg justify-center" title="{{ __('Download Image')}}"  href="{{ $image->imageUrl() }}" download="{{ $image->title }}">
                                                        <svg xmlns="http://www.w3.org/2000/svg" width="16"
                                                        height="16" viewBox="0 0 16 16" fill="none">
                                            ...
                                            <div class="relative xl:hidden inline-block">
                                                ...
                                                <div class="absolute ltr:right-0 rtl:left-0 mt-2 w-[201px] border border-color-89 dark:border-color-47 rounded-lg bg-white dark:bg-[#333332] z-50 table-drop-body dropdown-shadow table-dropdwon-rtl">
                                                    <div>
                                                        <a href="javascript:void(0)" class="image-modal-button flex justify-start items-center gap-1.5 text-14 font-normal text-color-14 dark:text-white font-Figtree px-4 py-2 hover:bg-color-F6 dark:hover:bg-[#3A3A39] rounded-t-lg text-left" data-name="{{ $image->title }}" data-promt="{{ $image->title }}" data-id="{{ $image->id }}" data-style="{{ data_get($image->generation_options,'art_style', 'None') }}" data-resulation="{{ data_get($image->generation_options,'size', 'None') }}" data-created="{{ $image->created_at }}" data-lightstyle="{{ data_get($image->generation_options,'light_effect', 'None') }}" data-is-favorite="{{ in_array($image->id, $userFavoriteImages) ? 'true' : 'false' }}" data-source="{{ $image->imageUrl(['thumbnill' => true, 'size' => 'medium']) }}" id={{ $image->id }} data-slug="{{ $image->slug }}">
                                                            <span class="w-4 h-4">
                                                        ...
                                                        <a href={{ $image->imageUrl() }} download={{ $image->title }} class="flex justify-start items-center gap-1.5 text-14 font-normal text-color-14 dark:text-white font-Figtree px-4 py-2 hover:bg-color-F6 dark:hover:bg-[#3A3A39] text-left">
                                                            <span class="w-4 h-4">
...
@section('js')
    ...
    <script src="{{ asset('Modules/OpenAI/Resources/assets/js/customer/image/gallery.min.js') }}"></script>
@endsection
